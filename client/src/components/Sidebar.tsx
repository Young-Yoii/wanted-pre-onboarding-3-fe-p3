    ...
    <div className="sidebar">
      ...
      <ul>
        {sidebarContent.map((element) => {
          return (
            <li key={element.path} className={currentPath === element.path ? 'sidebar-menu selected' : 'sidebar-menu'} onClick={() => sidebarMenuClickHandler(element.path)}>
              {element.label}
            </li>
          );
        })}
      </ul>
      <div>{userProfile ? <div className="sidebar-footer">{userProfile.userInfo.name}님 환영합니다.</div> : <div>로그인이 필요합니다.</div>}</div>
    </div>
  );
};
